...
public class DemoServlet extends HttpServlet {
	...
	// Auto wird hinzugefuegt
	private List<CarIF> cars(String[] params){
		List<CarIF> cars = Car.addcars(params,getCars());
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ( "cmd".equals( command ) && "sum".equals( param ) ){
			...
			String[][] stats	= Car.carStats(getCars());
			
			
			
			response.setContentType("text/html");
			PrintWriter out = response.getWriter();
			String ausgabe = BarChartBuilder.BuildBarChart(stats[3], new String[][]  {stats[0], stats[4]}, new String[] {"duration"});
				out.println(ausgabe);
